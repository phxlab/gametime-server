...
const isStoreOpen = (waves: { open: string; close: string }[]) => {
    ...
    for (const wave of waves) {
        ...
        const openTimeLosAngeles = DateTime.local(openYear, openMonth, openDay, { zone: 'America/Los_Angeles' });
        const closeTimeLosAngeles = DateTime.local(closeYear, closeMonth, closeDay, { zone: 'America/Los_Angeles' });
...
const storeOpenMiddleware = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
